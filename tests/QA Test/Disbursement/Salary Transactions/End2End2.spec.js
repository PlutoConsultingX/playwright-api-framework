...
test.describe.serial("Salary Transaction Workflow", () => {
  ...
  //   //1. Check if the callback_data field is not populated already from SAG
  //   // ----------------------------------
  ...
  //     console.log(
  //       `DB poll using correlationId (attempt ${i + 1}):`,
  //       JSON.stringify(dbResult, null, 2)
  //     );

  //     // ✅ Check if callback_data has a value
  ...
  //     if (rowWithCallback) {
  //       try {
  //         // 1️⃣ Extract callback_data STRING
  //         const callbackDataString = rowWithCallback.callback_data;

  //         console.log("Found callback_data STRING:", callbackDataString);

  //         // 2️⃣ Parse STRING → JSON
  //         const callbackDataJson = JSON.parse(callbackDataString);

  //         console.log("Parsed callback_data JSON:", callbackDataJson);
  //         console.log("Parsed callback_data JSON (FULL):", JSON.stringify(callbackDataJson, null, 2)
  //         );

  //         // 3️⃣ Extract required fields
  //         //cbTransactionId = callbackDataJson.transactionID;
  //         cbTransactionId = callbackDataJson.responseDetails.transactions[0].transactionId;
  //         cbNativeTransactionId = callbackDataJson.responseDetails.transactions[0].nativeTransactionID;
  //         cbSenderMessageId = callbackDataJson.isInfo.SenderMessageID;

  //         console.log("Extracted callback_data fields:", {
  //           cbTransactionId,
  //           cbNativeTransactionId,
  //           cbSenderMessageId,
  //         });

  //         break; // ✅ Stop polling once extracted
  //       } catch (error) {
  //         throw new Error(
  //           `callback_data found but failed to parse JSON: ${error.message}`
  //         );
  //       }
  //     }

  //     console.log(`No callback_data yet — retrying (${i + 1}/${maxRetries})`);
  ...
  //   console.log("Generated nativeTransactionId:", nativeTransactionId);
  ...
  //   const body =
  //     TransactionFactory.loadCreatedSalaryPayload({
  //       transactionId,
  //       senderMessageId,
  //       nativeTransactionId,
  //       correlationId,
  ...
  //   console.log(
  //     'Load Created Body (from factory):',
  //     JSON.stringify(body, null, 2)
  //   );
  //   //Intentionally waiting for 5 seconds as simulation processes too quick for the system to handle
  ...
  //   //const response = await TransactionFactory.loadCreatedEFTPayload(payload);
  //   //const body = await response.json();

  //   expect(response.status()).toBe(200);
  //   //expect(body.Response?.responseHeader?.responseCode).toBe("PROCESSED");
  ...
  //     const dbResult = await dbClient.query(
  //       loadCreatedQuery3.getSalaryTransactionFailedSAGResponsePostLoadCreatedMock,
  //       //loadCreatedQuery.getSalaryTransactionFailedSAGResponseAfterLoadCreatedMock,
  //       [correlationId]
  //     );


  //     console.log("This is the DB results after Load Created call with a Query that returns callback_data:", dbResult);
