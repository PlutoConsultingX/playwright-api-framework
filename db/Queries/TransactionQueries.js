  ...
  export const transactionQueries = {
    getSalaryTransactionFailedSAGResponse: `
  ...
  `,
  getSalaryTransactionPassedSAGResponse: `
    SELECT t.workflow_execution_id,t.transaction_id 
    ,ti.client_transaction_id ,ti.transaction_item_id 
    ,tidr.child_workflow_execution_id 
    ,ts.transaction_status_name as trxBatchStatus,tis.transaction_item_status_name as trxStatus,tids.transaction_item_detail_status_name as trxItemDetailStatus
    ...
    WHERE t.client_message_id = ?
  `,
  getSalaryTransactionProcessedToBankSAGResponse: `
    SELECT t.workflow_execution_id,
           t.transaction_id,
           ti.client_transaction_id,
           ti.transaction_item_id,
           tidr.child_workflow_execution_id,
           ts.transaction_status_name as trxBatchStatus,
           tis.transaction_item_status_name as trxStatus,
           tids.transaction_item_detail_status_name as trxItemDetailStatus,
           s.service_name,
           tid.transaction_item_detail_id,
           tidrs.transaction_item_detail_run_status_name,
           tidr.response_manager_code,
           tidr.response_manager_exception_type,
           tidr.response_manager_message,
           tidr.response_data,
           et.response_reference,
           et.request_payload_data as sagrequestpayload,
           et.transaction_id as senderID,
           et.transaction_run_detail_id as transactionIDSAGReqPayload,
           et.response_payload_data,
           et.response_type,
           et.transaction_status as workertransactionstatus
    FROM transaction t
    INNER JOIN transaction_status ts 
      ON t.transaction_status_id = ts.transaction_status_id
    INNER JOIN transaction_item ti 
      ON t.transaction_id = ti.transaction_id
    INNER JOIN transaction_item_status tis 
      ON ti.transaction_item_status_id = tis.transaction_item_status_id
    INNER JOIN transaction_item_detail tid 
      ON ti.transaction_item_id = tid.transaction_item_id
    INNER JOIN service s 
      ON tid.service_id = s.service_id
    INNER JOIN transaction_item_detail_status tids 
      ON tid.transaction_item_detail_status_id = tids.transaction_item_detail_status_id
    INNER JOIN transaction_item_detail_run tidr 
      ON tidr.transaction_item_detail_id = tid.transaction_item_detail_id
    INNER JOIN transaction_item_detail_run_status tidrs 
      ON tidr.transaction_item_detail_run_status_id = tidrs.transaction_item_detail_run_status_id
    INNER JOIN WORKFLOW_WORKER_EFT_PAYMENTS_SALARYDAY.salaryday_eft_payment_transaction et 
      ON et.transaction_run_detail_id = tidr.transaction_item_detail_run_id
    WHERE t.client_message_id = ?
      AND et.response_type = 'PROCESSED'
  `
};
